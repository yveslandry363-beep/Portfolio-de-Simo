...
const Work: React.FC<WorkProps> = ({ projects, experience }) => {
  ...
  const [visibleProjects, setVisibleProjects] = useState(4); // Start with 4 for grid balance
  ...
  useGSAP(() => {
    // Reveal Image Animation for Vertical List (Experience)
    ...
    // Refresh ScrollTrigger to ensure correct spacing after render
    ScrollTrigger.refresh();

  }, { scope: containerRef, dependencies: [filteredProjects, /* visibleProjects dependency included implicitly via re-render */ visibleProjects, experience] });
  ...
  return (
    <section id="work" ref={containerRef} className="relative z-10">
      ...
      {/* PART 2: Grid Layout (Projects) */}
      <div className="py-24 border-t border-white/10 bg-black/40 backdrop-blur-sm">
        <div className="px-6 md:px-12 max-w-7xl mx-auto">
          ...
          <div className="grid-container grid grid-cols-1 md:grid-cols-2 gap-8">
            {filteredProjects.length === 0 ? (
              <div className="col-span-full py-20 text-center text-gray-500 font-mono">No projects found for {filter}</div>
            ) : (
              filteredProjects.slice(0, visibleProjects).map((project, i) => (
                <div key={project.id} className="grid-item w-full h-[400px]">
                  <Link
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black via-black/80 to-transparent">
                      <div className="flex justify-between items-end">
                        <div className="flex-1 pr-4">
                          ...
                          <h3 className="text-2xl font-bold font-['Syne'] text-white mb-2"><TextScramble text={project.title} trigger={false} /></h3>
                          <p className="text-sm text-gray-400 line-clamp-2">{getLocalizedContent(project, 'description')}</p>
                        </div>
                        <div className="w-10 h-10 shrink-0 rounded-full border border-white/30 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all">
          ...
          {visibleProjects < filteredProjects.length && (
            <div className="mt-16 text-center">
              <p className="text-gray-400 mb-4 font-mono text-xs">{filteredProjects.length - visibleProjects} more projects...</p>
              <button
                onClick={loadMore}
                className="px-8 py-4 border border-white/30 text-white font-bold uppercase tracking-widest hover:bg-white hover:text-black hover:border-white transition-all rounded-full"
                data-cursor="link"
              >
                Load More Projects
              </button>
